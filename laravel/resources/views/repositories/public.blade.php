...
@section('content')
                            ...
                            <div class="d-flex">
                                @if(!$subscribed)
                                    @can('repository-subscribe')
                                        <form
                                            action="{{ route('repositories.subscribe', [$repository->id, $user->id]) }}"
                                            method="POST">
                                            @csrf
                                ...
                                @else
                                    @can('repository-unsubscribe')
                                        <form
                                            action="{{ route('repositories.unsubscribe', [$repository->id, $user->id]) }}"
                                            method="POST">
                                            @csrf
    ...
    @else
        <p>There is no repository available.</p>
    @endif
@endsection
